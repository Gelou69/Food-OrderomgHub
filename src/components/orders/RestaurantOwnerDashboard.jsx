// components/owner/RestaurantOwnerDashboard.jsx - ENHANCED CREATIVE VERSION
...
const StyledInput = (props) => (
    <input 
        className="w-full p-3 border rounded-lg bg-gray-50 text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-shadow" // Added transition-shadow
...
// ... OwnerAuthPage component remains the same (Sources 205-262) ...
...
const ProductSidebar = ({ productForm, setProductForm, handleProductSubmit, editingProduct, setShowProductModal }) => {
    ...
    // Ref for the sidebar to handle outside clicks
    ...
    return (
        <div className="fixed inset-0 bg-black/50 z-50 transition-opacity duration-300">
            <div ref={sidebarRef} className="fixed inset-y-0 right-0 w-full md:w-96 bg-white shadow-2xl p-6 transition-transform duration-300 transform translate-x-0 overflow-y-auto">
...
// ... (Auth and loading logic remains the same) ...
// --- FIX ENDS HERE ---
...
// ... (loadOrders function remains the same) ...
...
// ... (loadMyRestaurant logic remains the same) ...
    useEffect(() => {
...
let bg = '#F3F4F6';
        ...
        if (status === 'Preparing') { color = NAVY;
bg = '#E0F2F7'; } // Navy
        if (status === 'Driver Assigned' || status === 'Out for Delivery') { color = '#3B82F6';
bg = '#DBEAFE'; } // Blue
        if (status === 'Delivered' || status === 'Completed') { color = '#10B981';
bg = '#D1FAE5'; } // Green
                           ...
                           <div className="text-center py-20 opacity-50"><span className="text-5xl">📦</span><p className="mt-4 font-bold">No orders found.</p></div>
                            ...
                            <div className="space-y-4">
                              ...
                              {filteredOrders.map(order => (
                                    // ENHANCED: Hover animation for order card
                                    <div 
                                        key={order.id} 
...
tracking-wider">Restaurant Total</p>
        ...
        {/* The button is now redundant as click handles expansion, but keeping the visual toggle inside for clarity */}
                                            <button 
...
'▲' : '▼'}</span>
 ...
 {order.order_items.map((item, idx) => (
                           ...
                           <div className="flex items-center gap-2"><span className="bg-white px-2 py-0.5 rounded border font-bold text-xs">x{item.quantity}</span><span>{item.name}</span></div>
                                                            <span className="font-mono text-gray-600">₱{(item.price 
* item.quantity).toFixed(2)}</span>
 ...
 <div className="mt-5 flex gap-3">
              ...
              <button onClick={(e) => {e.stopPropagation(); updateOrderStatus(order.id, getNextStatus(order.status));}} className="flex-1 py-3 text-white rounded-lg font-bold hover:opacity-90 transition shadow-lg transform active:scale-[0.99]" style={{ backgroundColor: ORANGE }}>
...
order.status === 'Preparing') && ( 
                                                        <button onClick={(e) => {e.stopPropagation(); updateOrderStatus(order.id, 'Cancelled');}} className="px-4 py-3 bg-red-100 text-red-600 rounded-lg font-bold hover:bg-red-200 transition">Cancel Order</button>
...
(
          ...
          {product.image_url && <img src={product.image_url} alt={product.name} className="w-full h-48 object-cover object-center" />}
                                        <div className="p-4 relative">
                                            ...
                                            <button 
                                                ...
                                                className="absolute top-4 right-4 text-gray-400 hover:text-navy-500 transition"
                                                title="View Details"
                                            >
   ...
   <h3 className="font-extrabold text-xl mb-1">{product.name}</h3>
                                            ...
                                            <div className="flex 
justify-between items-center mb-4 border-t pt-3"><span className="font-bold text-2xl" style={{ color: ORANGE }}>₱{product.price}</span><span className={`text-sm font-semibold px-3 py-1 rounded ${product.stock > 10 ? 'bg-green-100 text-green-600' : product.stock > 0 ? 'bg-yellow-100 text-yellow-600' : 'bg-red-100 text-red-600'}`}>Stock: {product.stock}</span></div>
                                            <div className="flex gap-2">
 ...
 {/* Replaced Modal with Sidebar Component */}
                        {showProductModal && (
                            <ProductSidebar 
